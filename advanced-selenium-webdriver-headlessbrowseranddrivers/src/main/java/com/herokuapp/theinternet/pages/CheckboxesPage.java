	...
	public void selectAllCheckboxes() {
		...
		for (WebElement checkbox : checkboxes) {
			if (!checkbox.isSelected()) {
				checkbox.click(); // if it is click method we needed to pass locator but its webelemet so click is fine
			}
		}
	}
	
	//Verify all  available checkboxes are checked. If atleast once unchecked return false
	public boolean areAllCheckboxesChecked() {
		log.info("Verifyimg that all checkboxes are checked");
		List<WebElement> checkboxes = findAll(checkbox);
		for (WebElement checkbox : checkboxes) {
			if (!checkbox.isSelected()) {
				return false;
